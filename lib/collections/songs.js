...
function checkIfIpCanVote(ip, songId, callback) {
	...
	if (IPs.findOne({ip: ip, song_id: songId, date: {$gte: new Date(currentDateTime.getTime() - 1*1000*60*60)}})) {
		throw new Meteor.Error(403, 'You already vote this hour. Please try it later.');
	} else {			
		callback();
